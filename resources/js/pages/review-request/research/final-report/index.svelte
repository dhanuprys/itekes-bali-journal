...
<AppLayout {breadcrumbs}>
    <LayoutComposer>
        {#snippet header()}
            <Heading title="Laporan Akhir" description="Daftar laporan akhir penelitian Anda." />
        {/snippet}
            ...
            <Card.Root>
                ...
                <Card.Content>
                    {#if submissions.data.length === 0}
                        <div class="text-center py-10 text-muted-foreground">Belum ada penelitian yang masuk tahap laporan akhir.</div>
                    {:else}
                        <Table.Root>
                            <Table.Header>
                                <Table.Row>
                                                ...
                                                <DropdownMenu.Content align="end">
                                                    ...
                                                    {#if !hasSubmittedReport(submission)}
                                                        <DropdownMenu.Item
                                                            onclick={() =>
                                                                router.visit(
                                                                    route('apply.research.final_report.create', { submission_id: submission.id }),
                                                                )}
                                                        >
                                                            Buat Laporan
                                                        </DropdownMenu.Item>
                                                    {:else if submission.status === 'revision_needed'}
                                                        <DropdownMenu.Item
                                                            onclick={() => router.visit(route('apply.research.final_report.edit', submission.id))}
                                                        >
                                                            Revisi
                                                        </DropdownMenu.Item>
                                                    {/if}
                                                    {#if hasSubmittedReport(submission)}
                                                        <DropdownMenu.Item
                                                            onclick={() => router.visit(route('apply.research.final_report.show', submission.id))}
                                                        >
                                                            Detail
                                                        </DropdownMenu.Item>
                                                    {/if}
