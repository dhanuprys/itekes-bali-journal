<script lang="ts">
    ...
    import CommunityServiceTargetSheet from './community-service-target-sheet.svelte';
    import Heading from '@/components/heading.svelte';
    import * as AlertDialog from '@/components/ui/alert-dialog';
    import { untrack } from 'svelte';
    import { toast } from 'svelte-sonner';

    let { communityServiceTargets, filters } = $props();
    ...
    let targetToDelete: any = $state(null);
    ...
    const handleSearch = debounce((value: string) => {
        router.get(
            '/master/community-service-target',
            { search: value },
            {
                preserveState: true,
                replace: true,
    ...
    function confirmDelete() {
        if (targetToDelete) {
            router.delete(`/master/community-service-target/${targetToDelete.id}`, {
                onFinish: () => {
                    deleteDialogOpen = false;
                    targetToDelete = null;
                    toast.success('Target PKM berhasil dihapus');
                },
            });
        }
    }
...
<svelte:head>
    <title>Target PKM</title>
</svelte:head>

<AppLayout {breadcrumbs}>
    <LayoutComposer>
        {#snippet header()}
            <Heading title="Target PKM" description="Kelola data target pengabdian kepada masyarakat" />
        {/snippet}
        ...
        {#snippet filters()}
            <div class="relative w-full max-w-sm">
                ...
                <Input placeholder="Cari target PKM..." class="pl-8" bind:value={search} oninput={(e) => handleSearch(e.currentTarget.value)} />
            </div>
        {/snippet}
        ...
        <div class="rounded-md border bg-card">
            <Table.Root>
                ...
                <Table.Body>
                    {#if communityServiceTargets.data.length === 0}
                        <Table.Row>
                            <Table.Cell colspan={5} class="text-center h-24 text-muted-foreground">Tidak ada data target PKM.</Table.Cell>
                        </Table.Row>
                    {:else}
                        {#each communityServiceTargets.data as target, i (target.id)}
                            <Table.Row>
                                <Table.Cell>{(communityServiceTargets.current_page - 1) * communityServiceTargets.per_page + i + 1}</Table.Cell>
                                ...
                                <Table.Cell class="text-right">
                                    <DropdownMenu.Root>
                                        ...
                                        <DropdownMenu.Content align="end">
                                            ...
                                            <DropdownMenu.Item onclick={() => router.visit(`/master/community-service-target/${target.id}`)}
            ...
            <AlertDialog.Content>
                <AlertDialog.Header>
                    ...
                    <AlertDialog.Description>
                        Aksi ini tidak dapat dibatalkan. Ini akan menghapus target PKM <strong>{targetToDelete?.title}</strong> secara permanen.
                    </AlertDialog.Description>
                </AlertDialog.Header>
                <AlertDialog.Footer>
