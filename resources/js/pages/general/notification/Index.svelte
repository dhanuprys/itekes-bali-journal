    ...
    <LayoutComposer>
        ...
        {#snippet children()}
            <div>
                ...
                {:else}
                    <div class="space-y-6">
                        {#each Object.entries(groupedNotifications) as [key, group]}
                            {#if group.length > 0}
                                <div>
                                    <h3
                                        class="mb-2 px-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground/70 flex items-center gap-2"
                                    >
                                        {#if key === 'today'}
                                            Hari Ini
