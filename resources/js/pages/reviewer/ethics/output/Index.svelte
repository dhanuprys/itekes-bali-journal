...
<svelte:head>
    <title>Output Etik - Reviewer</title>
</svelte:head>

<AppLayout {breadcrumbs}>
    <LayoutComposer>
        {#snippet header()}
            <Heading title="Penerbitan Ethical Clearance" description="Unggah dokumen Ethical Clearance untuk pengajuan yang telah disetujui." />
        {/snippet}
                                    ...
                                    <Table.Row>
                                        ...
                                        <Table.Cell class="text-right">
                                            <Button variant="outline" size="sm" onclick={() => router.visit(route('review.ethics.wait_for_output.show', submission.id))}>
                                                {submission.latest_output?.document_path ? 'Lihat' : 'Upload EC'}
                                            </Button>
                                        </Table.Cell>
                                    </Table.Row>
                                {/each}
