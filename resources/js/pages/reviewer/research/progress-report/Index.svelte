...
<svelte:head>
    <title>Daftar Review Laporan Kemajuan</title>
</svelte:head>

<AppLayout {breadcrumbs}>
    <LayoutComposer>
        {#snippet header()}
            <Heading title="Review Laporan Kemajuan" description="Daftar laporan kemajuan penelitian yang perlu direview." />
        {/snippet}

        {#snippet children()}
            <Card.Root>
                <Card.Header>
                    <Card.Title>Daftar Laporan Kemajuan</Card.Title>
                    <Card.Description>Laporan yang ditugaskan kepada Anda.</Card.Description>
                </Card.Header>
                <Card.Content>
                    {#if submissions.data.length === 0}
                        <div class="text-center py-10 text-muted-foreground">Tidak ada laporan kemajuan saat ini.</div>
                    {:else}
                        <Table.Root>
                            <Table.Header>
                                <Table.Row>
                                ...
                                {#each submissions.data as submission}
                                    <Table.Row>
                                        ...
                                        <Table.Cell class="text-right">
                                            <Button size="sm" href={route('review.research.progress_report.show', submission.id)}>Review</Button>
                                        </Table.Cell>
                                    </Table.Row>
                                {/each}
                            </Table.Body>
