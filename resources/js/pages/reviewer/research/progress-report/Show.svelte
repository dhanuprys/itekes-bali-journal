    ...
    <LayoutComposer>
        {#snippet header()}
            <div class="flex items-center justify-between">
                <Heading title="Review Laporan Kemajuan" description="Tinjau laporan kemajuan penelitian berikut." />
        ...
        {#snippet children()}
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                    ...
                                    {#if detail?.final_report_path}
                                        <div class="flex items-center gap-3 p-3 border rounded bg-muted/50">
                                            ...
                                            <div class="flex-1 overflow-hidden">
                                                <p class="text-smtruncate">File Laporan</p>
                                            </div>
                ...
                <!-- Right Column -->
                <div class="space-y-6">
                    <CommentSection {comments} submitRoute={route('review.research.progress_report.comment', submission.id)} />

                    <ReviewActions {canReview} submitRoute={route('review.research.progress_report.change-state', submission.id)} />
                </div>
            </div>
        {/snippet}
    </LayoutComposer>
